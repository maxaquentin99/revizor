...
export default class Admin  extends React.Component {
  ...
  saveUser = async (questions, employees) => {
    try {
      this.setState({questions: questions});
      this.setState({employees: employees});
      let res = await axios.post('https://revizor.space/api/update/questions/', {
        ...this.state.client,
        ...
        {employees.length > 0 &&
          ...
          employees.map((employee, i) => { return (
            <div key={i}>
              <Grid container spacing={3} alignItems="flex-end" justify="center">
              <Grid item>
              <img key={'ava'+i} src={'https://revizor.space/avatars/'+employee.img} alt={employee.name} className="employee-ava" />
              </Grid>
              <Grid item>
              <TextField
              label="Name"
